export const ANIMAL_PROTEINS = new Set(['beefPatty', 'grilledChicken', 'crispyChicken']);
export const ANIMAL_CHEESES = new Set(['cheddar', 'emmental', 'mozzarella', 'pepperCheese']);
...
export function isVeganBurger(state: Record<string, string | null>): boolean {
  ...
  const hasAnimalProtein = proteinSlots.some((s) => state[s] && ANIMAL_PROTEINS.has(state[s]!));
