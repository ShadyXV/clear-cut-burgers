...
Single-page React 19 app (Vite, Tailwind v4, Motion/React) with no router. All navigation is pure state managed in `App.tsx`.

### View routing

`App.tsx` owns a `view` state (`'builder' | 'impact' | 'deaths'`) and a `VIEW_DEPTH` map. Slide direction is computed from depth comparison so forward navigation slides left-to-right and backward slides right-to-left. `AnimatePresence mode="wait"` wraps the three views.

`CheckoutTransition` is intentionally placed **outside** `AnimatePresence` — it must persist as an overlay while the builder→impact swap happens underneath it.
